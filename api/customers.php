...
try {
    ...
    // Buscar todos os clientes (usuários)
    $stmt = $pdo->prepare("SELECT * FROM users ORDER BY id ASC");
    $stmt->execute();
    $customers = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    // Se não houver tabela users, retornar array vazio
    if (empty($customers)) {
        $customers = [];
    }
    
    // Retornar clientes em JSON
    echo json_encode($customers, JSON_UNESCAPED_UNICODE);
    
} catch(PDOException $e) {
    // Em caso de erro, retornar array vazio ou erro em JSON
