...
const AdminOrders = () => {
  ...
  // 🎯 FUNÇÃO PARA BUSCAR PEDIDOS DO MYSQL
  const fetchOrders = async () => {
    try {
      setLoading(true)
      console.log('🔄 Buscando pedidos do MySQL...')
      
      const response = await fetch('http://localhost:8000/orders.php')
      
      if (!response.ok) {
        throw new Error('Erro ao conectar com MySQL')
      }
      
      const mysqlOrders = await response.json()
      console.log('✅ Pedidos recebidos do MySQL:', mysqlOrders)
      
      // 📊 TRANSFORMAR DADOS DO MYSQL
      const transformedOrders = Array.isArray(mysqlOrders) ? mysqlOrders.map(order => ({
        id: order.id,
        orderNumber: order.order_number || `ORD-${String(order.id).padStart(3, '0')}`,
        customer: {
          name: order.customer_name || order.customer || 'Cliente',
        ...
        createdAt: order.created_at || order.date || new Date().toISOString(),
        completedAt: order.completed_at || null,
        deliveryAddress: order.delivery_address || '',
        notes: order.notes || ''
      })) : []
      
      setOrders(transformedOrders.length > 0 ? transformedOrders : mockOrders)
      console.log('✅ Pedidos carregados do MySQL:', transformedOrders.length)
      setLoading(false)
      ...
      console.error('❌ Erro ao carregar pedidos do MySQL:', error)
      
      // Fallback para dados mockados enquanto tenta reconectar
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="mb-6 sm:mb-8 lg:mb-10 pt-8 sm:pt-12 lg:pt-16 xl:pt-20">
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 sm:gap-6 lg:gap-8">
          <div className="flex-1 min-w-0 space-y-3 sm:space-y-4">
            <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-semibold text-gray-800 leading-tight tracking-wide">
              Gestão de Pedidos
            </h1>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 sm:gap-5 lg:gap-6 mb-6 sm:mb-8">
          <div className="bg-white rounded-xl p-4 sm:p-5 lg:p-6 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              <div className="flex-1 min-w-0">
                <p className="text-xs sm:text-sm text-gray-600 font-medium mb-1">Total de Pedidos</p>
                <p className="text-xl sm:text-2xl font-bold text-gray-900">{stats.total}</p>
              </div>
              <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-blue-100 to-blue-200 rounded-lg flex items-center justify-center flex-shrink-0 ml-2">
          ...
          <div className="bg-white rounded-xl p-4 sm:p-5 lg:p-6 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              <div className="flex-1 min-w-0">
                <p className="text-xs sm:text-sm text-gray-600 font-medium mb-1">Concluídos</p>
                <p className="text-xl sm:text-2xl font-bold text-green-600">{stats.completed}</p>
              </div>
              <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-green-100 to-green-200 rounded-lg flex items-center justify-center flex-shrink-0 ml-2">
          ...
          <div className="bg-white rounded-xl p-4 sm:p-5 lg:p-6 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              <div className="flex-1 min-w-0">
                <p className="text-xs sm:text-sm text-gray-600 font-medium mb-1">Processando</p>
          ...
          <div className="bg-white rounded-xl p-4 sm:p-5 lg:p-6 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              <div className="flex-1 min-w-0">
                <p className="text-xs sm:text-sm text-gray-600 font-medium mb-1">Pendentes</p>
                <p className="text-xl sm:text-2xl font-bold text-yellow-600">{stats.pending}</p>
              </div>
              <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-yellow-100 to-yellow-200 rounded-lg flex items-center justify-center flex-shrink-0 ml-2">
          ...
          <div className="bg-white rounded-xl p-4 sm:p-5 lg:p-6 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              ...
              <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-purple-100 to-purple-200 rounded-lg flex items-center justify-center flex-shrink-0 ml-2">
      ...
      <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-5 lg:p-6 border border-gray-100 shadow-sm mb-6 sm:mb-8">
            ...
            <AnimatePresence>
              {sortedOrders.map((order, index) => (
                <motion.div
                  ...
                  className="bg-gray-50 rounded-xl p-4 sm:p-5 lg:p-6 hover:bg-gray-100 transition-all duration-200 border border-gray-100"
                >
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-4">
                    <div className="flex items-center space-x-3 sm:space-x-4">
                      ...
                      <div className="min-w-0 flex-1">
                        <h4 className="font-semibold text-sm sm:text-base text-gray-900 truncate">{order.orderNumber}</h4>
                        <p className="text-xs sm:text-sm text-gray-500 truncate">{formatDate(order.createdAt)}</p>
                      </div>
                    </div>
                    
                    <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                      <span className={`inline-flex items-center px-2 sm:px-3 py-1 rounded-full text-xs font-medium border ${getStatusInfo(order.status).color}`}>
                        {getStatusInfo(order.status).icon}
                        <span className="ml-1 hidden sm:inline">{getStatusInfo(order.status).text}</span>
                      </span>
                      
                      <span className={`inline-flex items-center px-2 sm:px-3 py-1 rounded-full text-xs font-medium border ${getPaymentStatusInfo(order.paymentStatus).color}`}>
                        {getPaymentStatusInfo(order.paymentStatus).text}
                      </span>
                    </div>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 lg:gap-6 mb-4">
                    <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4 p-4 bg-white rounded-lg border border-gray-100">
                    <div>
