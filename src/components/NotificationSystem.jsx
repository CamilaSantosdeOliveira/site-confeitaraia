...
const NotificationProvider = ({ children }) => {
  ...
  const cart = useCallback((title, message, duration = 4000) => {
    return addNotification({ type: 'cart', title, message, duration })
  }, [addNotification])

  const wishlist = useCallback((title, message, duration = 4000) => {
    return addNotification({ type: 'wishlist', title, message, duration })
  }, [addNotification])

  const promotion = useCallback((title, message, duration = 8000) => {
  ...
  const value = useMemo(() => ({
    ...
    wishlist,
    promotion
  }), [notifications, addNotification, removeNotification, success, error, warning, info, cart, wishlist, promotion])
